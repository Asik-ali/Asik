    ...
    <section className="py-8 overflow-x-hidden">
      <div className="max-w-screen-xl mx-auto px-4  text-gray-600 md:px-8">
        <div className="max-w-2xl mx-auto">
          <h3 className="text-black text-3xl font-semibold sm:text-4xl text-center">
            Why Choose Us
          </h3>
          ...
          <ul className="grid gap-y-8 gap-x-12 sm:grid-cols-2 lg:grid-cols-3">
            {features.map((item, idx) => (
              <li key={idx} className={`space-y-3 p-4 ${idx === 0 ? 'bg-[#E7E5FD]' : idx === 1 ? 'bg-[#FFDEE9]' : 'bg-[#FDF5E0]'} text-black font-semibold transition-colors duration-300 ease-in-out`}>
                <div className={`w-12 h-12 mx-auto ${idx === 0 ? 'bg-[#B0ACFF]' : idx === 1 ? 'bg-[#FF80AB]' : 'bg-[#FECC49]'} text-white rounded-full flex items-center justify-center`}>
                  {item.icon}
                </div>
